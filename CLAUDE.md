...
- **Build optimization**: Default esbuild minification (no `console.*`/`debugger` stripping — `console.error` is intentionally kept so server API routes log to Vercel). No manual chunk groups: Rolldown's `codeSplitting.groups` also capture each match's dependencies, which once pulled mermaid's graph code onto every page. Leave splitting on defaults.
